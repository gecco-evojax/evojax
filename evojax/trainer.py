...
from evojax.algo import ESAlgorithm
...
class Trainer(object):
    ...
    def __init__(self,
        ...
        Args:
            ...
            demo - Whether to test the learned model only.
    ...
    def run(self) -> float:
        ...
        if self._model_dir is not None:
            params, obs_params = load_model(model_dir=self._model_dir)
            self.sim_mgr.obs_params = obs_params
            self._logger.info(
                'Loaded model parameters from {}.'.format(self._model_dir))
        else:
            params = None
        ...
        if self._demo_mode:
            ...
            if params is not None:
                ...
                self._solver.best_params = params
            ...
            for i in range(self._max_iter):
                ...
                start_time = time.perf_counter()
                self._solver.tell(fitness=scores)
                ...
                if i > 0 and i % self._test_interval == 0:
                    best_params = self._solver.best_params
