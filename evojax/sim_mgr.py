...
@jax.jit
def split_states_for_pmap(state: TaskState) -> TaskState:
...
class SimManager(object):
    ...
    def __init__(self,
        ...
        def step_once(carry, input_data, task):
            state, params, obs_params, accumulated_reward, valid_mask = carry
            if task.multi_agent_training:
                num_tasks, num_agents = state.obs.shape[:2]
                obs = state.obs.reshape((-1, *state.obs.shape[2:]))
            else:
                obs = state.obs
            normed_obs = self.obs_normalizer.normalize_obs(obs, obs_params)
            actions = policy_net.get_actions(normed_obs, params)
            if task.multi_agent_training:
            ...
            return ((state, params, obs_params, accumulated_reward, valid_mask),
                    (state.obs, valid_mask))

        def rollout(states, params, obs_params, step_once_fn, max_steps):
            accumulated_rewards = jnp.zeros(params.shape[0])
            valid_masks = jnp.ones(params.shape[0])
            ((states, params, obs_params, accumulated_rewards, valid_masks),
        ...
        if self._num_device > 1:
            self._train_rollout_func = jax.pmap(
        ...
        if self._num_device > 1:
            self._valid_rollout_func = jax.pmap(
    ...
    def eval_params(self, params: jnp.ndarray, test: bool) -> jnp.ndarray:
        ...
        # Reset the tasks.
        state = reset_func(reset_keys)
        if self._num_device > 1:
            params = split_params_for_pmap(params)
            state = split_states_for_pmap(state)

        # Do the rollouts.
        scores, all_obs, masks = rollout_func(state, params, self.obs_params)
        if self._num_device > 1:
